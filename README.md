...
Install: `npm i @omkar273/mcp-server`. Or run with one command (no clone or build):

```bash
npx @omkar273/mcp-server start --server-url https://api.flexprice.io --api-key-auth YOUR_API_KEY
```
...
3. Create a `.env` file (from `.env.example` if present) with:
   - `BASE_URL=https://api.flexprice.io` (must include `/v1`; no trailing space or trailing slash)
...
docker run -i -e API_KEY_APIKEYAUTH=your_api_key_here -e BASE_URL=https://api.flexprice.io flexprice-mcp node bin/mcp-server.js start
```
...
```bash
claude mcp add flexprice -- npx -y @omkar273/mcp-server start --server-url https://api.flexprice.io --api-key-auth YOUR_API_KEY
```
...
```json
{
  "mcpServers": {
    "flexprice": {
      ...
      "env": {
        "API_KEY_APIKEYAUTH": "your_api_key_here",
        "BASE_URL": "https://api.flexprice.io"
      }
    }
  }
}
...
```json
"args": ["-y", "@omkar273/mcp-server", "start", "--server-url", "https://api.flexprice.io", "--api-key-auth", "YOUR_API_KEY", "--mode", "dynamic"]
```
...
Omkar MCP tools are categorized into three permission scopes to allow fine-grained access control:
...
- **Fix:** Set `BASE_URL=https://api.flexprice.io` (no trailing space or slash after `v1`). For npx, pass `--server-url https://api.flexprice.io`.
- If you get **404** on tool calls, ensure the base URL includes `/v1`.

### API connection issues

1. **Credentials:** Check that your API key and base URL are correct. Test the key with the flexprice API (e.g. `curl -H "x-api-key: your_key" https://api.flexprice.io/customers`).
